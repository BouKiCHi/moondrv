...
トラック記号は行頭の文字で指定します。
英字の1文字に対して1つのトラックが割り当てられています。
1トラックに対して1つの音が割り当てられています。

デフォルトではOPL4の波形音源がトラックA-Xに割り当てられます。

以下の指定はトラック記号に影響を与えます。
...
FM音源部分を有効にします。
小文字のaからrまでが波形音源になります。

## OPL4 波形音源無効フラグ (OPL3/OPL4)

```
#OPL4-NOUSE
```

波形音源無効フラグです。この定義が記述されている場合、
FM音源トラックがAから開始され、波形音源のトラック処理がなくなり、
ドライバの負担が軽減されます。
...
OPL4との同時使用ではトラック記号は小文字になります。

### トラックとスロットの関係


| トラック記号(2OP) | A | B | C | D  | E  | F  | G  | H  | I  |
| ----------------- | - | - | - | -- | -- | -- | -- | -- | -- |
| チャンネル        | 1 | 2 | 3 | 4  | 5  | 6  | 7  | 8  | 9  |
| チャンネル(4OP時) | 1 | 2 | 3 | 1  | 2  | 3  |    |    |    |
| スロット1         | 1 | 2 | 3 | 7  | 8  | 9  | 13 | 14 | 15 |
| スロット2         | 4 | 5 | 6 | 10 | 11 | 12 | 16 | 17 | 18 |


| トラック記号(2OP) | J  | K  | L  | M  | N  | O  | P  | Q  | R  |
| ----------------- | -- | -- | -- | -- | -- | -- | -- | -- | -- |
| チャンネル        | 10 | 11 | 12 | 13 | 14 | 15 | 16 | 17 | 18 |
| チャンネル(4OP時) | 4  | 5  | 6  | 4  | 5  | 6  |    |    |    |
| スロット1         | 19 | 20 | 21 | 25 | 26 | 27 | 31 | 32 | 33 |
| スロット2         | 22 | 23 | 24 | 28 | 29 | 30 | 34 | 35 | 36 |
...
```
@TONE<MML音色番号> = {
　最低音階, 最高音階, PCM音色番号, ピッチ補正値,
　LFO & VIB, AR & D1R, DL & D2R, RC & RR, AM
}
```

二行目はOPL4各レジスタに対応して、この定義がPCM内の定義よりも優先されます。
...
@OPLと@OPFの音色番号は共用になりますので、重複はできません。
...
## ■OPM - 4OP コネクションセレクタ設定(OPL3)

| 値      | D5    | D4    | D3    | D2  | D1  | D0  |
| ------- | ----- | ----- | ----- | --- | --- | --- |
| 4OP Trk | L     | K     | J     | C   | B   | A   |
| 4OP CH  | 6     | 5     | 4     | 3   | 2   | 1   |
| 2OP CH  | 12,15 | 11,14 | 10,13 | 3,6 | 2,5 | 1,4 |

OPMの各列はビットになります。
1にすると4OPチャンネルが有効になり2OPチャンネルが使用できなくなります。
4OP Trkは4OP時のトラック記号です。
...
CON = ((&#x5b;C0&#x5d;)&1)<<1 + (&#x5b;C3&#x5d;)&1とした場合

| 定義 | ST                  | VOP  | TVP  |
| ---- | ------------------- | ---- | ---- |
| @OPL | CONはbit0とbit1が逆 | あり | なし |
| @OPF | CONと同じ           | なし | あり |
