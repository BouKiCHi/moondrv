...
// MDRファイルのあるディレクトリを取得
void setDirNameFromPath(char *dest, const char *path) {
  strcpy(dest, path);
  char *p = strrchr(dest, PATH_DELIM);
  // ディレクトリ名が存在しない
  if (!p) { dest[0] = 0x00; return; } 
...
int readMDRHeader(const char *file, _mdr *m) {
  if (openMDRFile(file, "rb", m) < 0) return -1;
...
int main(int argc, char *argv[]) {
  ...
  if (!pcmfile) {
    ...
    if (m.pcmname[0]) {
      setDirNameFromPath(pcmfile, mdrfile);
